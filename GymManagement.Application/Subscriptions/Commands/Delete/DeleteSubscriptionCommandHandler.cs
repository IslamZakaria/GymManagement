...
{
    internal sealed class DeleteSubscriptionCommandHandler : IRequestHandler<DeleteSubscriptionCommand, ErrorOr<Deleted>>
    {
        ...
        {
            var subscription = await _subscriptionsRepository.GetByIdAsync(command.Id);
            ...
            admin.DeleteSubscription(command.Id);

            var gymsToDelete = await _gymsRepository.ListBySubscriptionIdAsync(command.Id);
